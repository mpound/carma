# $Id: bandpassCheck.py,v 1.5 2012/12/19 16:42:42 obs Exp $
#
# To use:
#
#   run('bandpassCheck')
...
#
# History
# 22-Oct-2010: PJT      original version, cloned from phasetest
#  2-Apr-2012: PJT      hardcoded test for different windows
#
"""
    bandpassCheck.py
    ...
    Purpose: 
        ...
        and used for bandpass stability tests

    Instructions:
        The script should be run from the sci1 subarray as follows:
           Sci#1[1]: run('bandpassCheck')
        ...
        The script accepts several command line options:
           ...
           tune=False     : Do not tune the receivers and do not reconfigure 
                            the correlator
         Examples:
             Sci#1[1]: run('bandpassCheck', tune=False, source='3c273')
             Sci#1[2]: run('bandpassCheck', fsky=230.0, pnt=True)
...
p.add('time',   default=0.25,  type=float, description='Length of track (hours)')
...
# Project parameters
trackFilename = 'bandpassCheck.py' 
projectCode   = 'ct003'
obsblock      = 'bandpass'
pi            = 'Array Health'
...
cc            = None
bcc           = None
subject       = 'bandpass stability test'
...
if p.tune:
   ...
   if True:
       ...
       commands.configastroband(2, "LL", commands.BW125, p.fsky)
       commands.configastroband(3, "LL", commands.BW62,  p.fsky)
       commands.configastroband(4, "LL", commands.BW31,  p.fsky)
       commands.configastroband(5, "LL", commands.BW8,   p.fsky)
       commands.configastroband(6, "LL", commands.BW2,   p.fsky)
       commands.configastroband(7, "LL", commands.BW31,  p.fsky)
       commands.configastroband(8, "LL", commands.BW500, p.fsky)
