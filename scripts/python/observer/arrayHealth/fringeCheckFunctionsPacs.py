...
def getHtmlHeader():
    ...
    htmlHeader += '</head>\n'
    htmlHeader += '<body background="/observing/bkground1.gif">\n'
...
def createHtmlResults(var, antennas):
    ...
    if not os.path.exists(var['fullOutputDir']): 
        os.system('mkdir ' + var['fullOutputDir'])
    if not os.path.exists(var['fullOutputDir']): 
    ...
    gifFiles = checkFiles(var)

    # Move fringe check log file
    com = "mv " + var['visdataDir'] + '/' + var['fringeLog'] + ' ' + \
    ...
    com  = getHtmlHeader()
    com += '<table border=0>\n'
    com += '<TR><TH align=left>UT Date of observations:</TH><TD>' + sutdate + \
           '</TD></TR>\n'
    ...
    print "run('fringeCheck', obsblock='" + var['obsblockName'] + "')"
    print "********************************************************************"
    ...
    # Move default link
    os.system('cd ' + var['fringeDir'] + '; ln -s ' + var['htmlDir'] + \
              '/index.html index.html' + '; chmod 777 index.html')

def observeSources(var, p):
    """ Observe noise and astronomical source
    """
    # Initialize but only clearastrobands if we are tuning too
    ...
    if p.mm1:
        fsky = 225.0
    flo = fsky - p.iffreq
    if sb == commands.LSB: flo = fsky + p.iffreq
    ...
    flux = utils.getSourceFlux(var['source'], freq=fsky)

    # Project
    projectCode = 'fringe'
    var['obsblock'] = var['source'].split('.')[0]
    ...
    commands.newProject(projectCode, var['obsblock'], var['subObsblockName'])
    commands.intent(var['source'],'G',True,False)
    commands.intent('noise','P',False,False)
    ...
    if p.tune:
        # Set correlator
         ...
         commands.configband(1, commands.BW500, fsky-0.5)
         commands.configband(2, commands.BW500, fsky)
         commands.configband(3, commands.BW500, fsky+0.5)
         commands.configband(4, commands.BW500, 'offline')
         commands.configband(5, commands.BW500, 'offline')
         print 'Tuning to flo = ' + str(flo) + ' GHz'
         sb = commands.USB
         if p.sb == 'LSB': sb = commands.LSB
         commands.freq(fsky, sb, p.iffreq, var['source']) 
         commands.checkbands()
    else : 
         print "doing tsys"
         commands.tsys()
    ...
    # Noise source for pacs. Must be done after CARMA noise
    # source so I can use CARMA to determine UT time stamps
    if p.pacs:
       print '\nStarting SZA noise integration'
       pacs.noise(combine)

    # Point up if needed
    if p.pnt: rp.refPoint(var['source'])

    # Bright source
    if p.pacs: 
       print '\nSlewing SZA antennas'
       pacs.observe(var['source'], sza_intent)
       pacs.tsys(combine)
    commands.track(var['source'])
    tint = p.tint
    ...
    print 'obsblock = ',var['obsblockName']
...
def runFiller(var):
    ...
    """
    # now do a loop: every 10 seconds check if xml file is done, then go;
    # after 3 minutes give up waiting and try myself.
    # as test, check whether the file has not filled so still .xml.write status
    iwait=0
    xmldir  = '/opt/sdp/astroheader/SLCorrelIntegrated/'
    xmlfile = xmldir + '/astrohdr_' + var['obsblockName'] + '.xml.*'
    while (iwait < 180):
       result = glob.glob(xmlfile)
       if list in [type(result)] and len(result) > 0 : break
       commands.sleep(5)
       if iwait == 0: 
           print '\nLooking for the XML file before running the filler...'
           print '\nFile = ' + xmlfile
       iwait += 5
    if iwait > 0: print 'Waited ',iwait,' seconds for the xml file to appear\n'
    ...
    print 'Running manual fill on ' + var['obsblockName']
    print 'Output file written to ' + var['visdataDir']
    mirfile = var['visdataDir'] + '/' + var['obsblockName'] + '.mir';
    os.system('rm -rf '+ mirfile + ';' + var['Filler'] + ' ' + var['obsblockName'] + ' >& /dev/null; chmod 777 ' + mirfile)


def runFringeCheck(var, referenceAntenna=None, pacs=False):
    ...
    com = 'cd ' + var['visdataDir'] + '; ' + 'listobs vis=' + miriadFile + \
    ...
    while not completed and n < len(ant_online):
        ...
        # Set command
        com = 'cd ' + var['visdataDir'] + '; ' + \
             var['fringeCheck'] + ' vis=' + miriadFile + ' output=' + \
             var['output'] + ' refant=' + str(refant) + \
             ' cm=' + str(int(var['cm'])) + \
             ' pacs=' + str(int(pacs))
        (dummy, fout) = os.popen4(com)
        ...
        outputLog = var['visdataDir'] + '/' + var['fringeLog']
        f = open(outputLog, 'w')
        f.write(resultsLog)
        f.close()
    ...
    # Remove miriad file
    com = 'rm -rf ' + var['visdataDir'] + '/' + miriadFile
    os.system(com);

    # Return antennas
    return [ant_withdata, ant_nodata_online, ant_nodata_offline, 
            ant_online, utdate, miriadError]
...
def checkFiles(var):
    ...
    root = var['visdataDir'] + '/' + var['obsblockName']
    ...
    # Add source spectrum plot - passband
    # "nameOrig" is the name of the file created by the miriad script
    # "label" is the label used on the web page
    plot = dict()
    plot['nameOrig']  = root + '.mir_source_gpplt_pb.gif'
    plot['name']  = 'source_gpplt_pb.gif'
    ...
    for f in gifFiles:
       ...
       if gifFiles[f]['exists'] and \
          os.system('mv -f ' + gifFiles[f]['nameOrig'] + ' ' + \
...
def createArchiveTime(var, dirsAll=None, nmax=None, header=None):
    ...
    for d in dirsAll:
        ...
        # Print entry
        nfiles += 1
        com += '<TR><TH bgcolor="#C0FFC5">%d</TH><TD>%s</TD><TD>%s</TD><TD><a href="%s/results/%s/index.html">%s</a></TD><TD>%s</TD>\n' % \
            (nfiles, ut_date, ut_time, var['fringeDirHtml'], d, sourcename, trial)
...
def createArchiveCalendar(var, dirsAll):
    ...
    for y in years:
        # Start year
        com += '<TR><TH colspan=13><H2>2007</h2></TH></TR>\n'
        com += '<TR><TH>Day</TH>'
        for m in months:
            com += '<TH>' + m + '</TH>'
        com += '</TR>\n'
