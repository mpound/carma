...
# $Id: fluxcal_obsdef3.py,v 1.6 2013/12/19 17:53:37 scott Exp $
# 2011-Apr-15  WK     fluxcal_obsdef3.py: added more options for fluxcal in
#                     readOptions()
...
def initializeTrack(sources,  mosaic,   projectInfo, limits,  
    ...
    if restart: 
        ...
        # j = MIN_INDX_HISTORY
        # if commands.s.getScriptInt(INDX_INT_HISTORY) > 0: 
        #     j = commands.s.getScriptInt(INDX_INT_HISTORY)
    ...
    if restart and \
       obsblockName != commands.s.getScriptString(INDX_STR_OBSBLOCK_ID):
        if var['resetObsblock']:
           ...
           # Check obsblocks name match if pacs
           if runPacs:
              # Reset sci2 obsblock
              ...
              # make sure subobsblock is 'PACS'
              if len(sci2Obsblock) != 4 or sci2Obsblock[2] != 'PACS':
              ...
              for i in range(len(i1)):
                 if sciObsblock[i1(i)] != sciObsblock[i2(i)]:
    ...
    if runPacs:
       obsblockNameSci2 = commands.queryString('Control.Subarray2.obsblockid')
        ...
        if not pointed and not observedFluxcal and not observedPassband and \
           utils.continueObservations(isScience=True) and tremaining > 0.0:
            ...
            for i in range(len(sou)):
                ...
                # Observe source
                if mosaic_source:
                    ok = observeMosaic(sou[i], t[i], mosaic, sources,
...
def observeMosaic(souMosaic, integTimeMosaic, mosaic, sources, lststop=None, 
                  ...
                  intent='S', pointstatus=commands.ONSRC):
    """ Make mosaic
    ...
    if utils.timeRemaining(source=souMosaic, lsttime=lststop) > 0:
        ...
        while ((maximumTime == None or 
            ...
            if tintSinceLastTsys == None or \
                   (sources['tsys'] <> None and 
                   tintSinceLastTsys >= abs(sources['tsys'])) :
                commands.trackMessage('Measuring tsys', indent=1)
                # Wait for antennas to be at final elevation
                commands.wait(commands.TRACK,tmo=500,
                              waiton=commands.ALL, subarray=sa)
                commands.tsys()
                ...
                if mosaic.has_key('off') and mosaic['off'] <> None:
                    ...
                    nreps = utils.getNreps(tint, sources['record'], tsampleMinutes=tsampleNoise)
                    ...
                    commands.integrate(sources['record'], nreps, 
             ...
             if phaseCalOrig == None:
                 ...
                 if autoPhase[tuneIndx]:
                     ...
                     phaseCal = utils.getSource(sourceList, ref=sou_snap,
                                     ...
                                     maxsep=autoSep, minsepsun=sources['minsepsun'])
                     if phaseCal == None:
