...
# $Id: obsdef3.py,v 1.179 2015/03/17 15:47:53 jmc Exp $
...
def readRestartVariables(var, mosaic, pointing):
    ...
    var['lowcoh']    = commands.s.getScriptString(INDX_STR_LOWCOH)
    ...
    nocameras = commands.s.getScriptString(INDX_STR_OPT_NOCAMERAS)
    ...
    if sopt != "": 
        ...
        # No cameras
        if not pointing.has_key('noCameras'):
            pointing['noCameras'] = list()
            if nocameras != '': pointing['noCameras'] = utils.makeListObsdef(nocameras)
...
def setHiddenOpticalPointing(pointing):
    ...
    pointing['nreps'] = utils.getVariable(pointing,'nreps',1,nonone=True)
...
def setHiddenParameters(limits, fluxcal, mosaic, passband, pointing, 
    ...
    pointing['reconfigure'] = utils.getVariable(pointing,'reconfigure', None)
    if not fluxcal.has_key('doPoint'):   fluxcal['doPoint']  = True
    if not passband.has_key('doPoint'):  passband['doPoint'] = False
    param = ['maxsep']
    value = [pointing['maxsep']]
    ...
    passband['allowShadowingPreferred'] = utils.getVariable(passband,'allowShadowingPreferred',False)
    passband['diameterFraction'] = utils.getVariable(passband,'diameterFraction',False)
    ...
    # Mosaic target - this must be done before the snapshot check
    sources['mosaicTarget'] = utils.setSourceVariable(sources['target'], sources['mosaicTarget'], False)[1]
    ...
    sources['snapTimeBeforeSet'] = utils.getVariable(sources,'snapTimeBeforeSet',None)
    sources['snapPreferComplete'] = utils.getVariable(sources,'snapPreferComplete',False)
    sources['snapRequireComplete'] = utils.getVariable(sources,'snapRequireComplete',False)
    sources['snapVerboseSelection'] = utils.getVariable(sources,'snapVerboseSelection',False)
    ...
    sources['snapFollow'] = utils.getVariable(sources,'snapFollow', False)
    ...
    sources['snapProject'] = utils.setSourceVariable(sources['target'], sources['snapProject'], None)[1]
    sources['snapDb'] = utils.getVariable(sources, 'snapDb', sn.DB_DEFAULT)
    sources['snapDbMosaic'] = utils.getVariable(sources, 'snapDbMosaic', sn.DB_MOSAIC_DEFAULT)
    sources['snapTintSecondarycal'] = utils.getVariable(sources, 'snapTintSecondarycal', 1)
    sources['snapElMinUseable'] = utils.getVariable(sources, 'snapElMinUseable', 0)
    sources['snapElMaxUseable'] = utils.getVariable(sources, 'snapElMaxUseable', 90)

    # Sources
    if sources['doSnapshot']: checkSnapshotSources(sources, mosaic)
    ...
    mosaic['otfUserlist']    = utils.getVariable(mosaic, 'otfUserlist', None)

def createOtfMosaic(mosaic, i):
   """ Creates OTF mosaic using parameters stored in the mosaic dictionary """
   # Note: if userlist is given all other grid parameters are ignored.
   return commands.Mosaic(mosaic['otfFieldtime'][i], mosaic['otfRows'][i], 
...
def setMosaicParameters(mosaic, sources):
    ...
    keywords = ['otfFieldtime',    'otfRows',     'otfFieldsInRow', 'otfDelta',
                ...
                'otfTrkThreshold', 'otfSlewtime', 'otfRowOffset',   
                'otfColOffset',    'otfUserlist',
    ...
    mosaic['otfMosaic'] = []
    for i in range(ntargets):
       if mosaicTarget[i] and mosaic['otf'] and sources['doSnapshot'] == False:
          commands.trackMessage('Initializing mosaic for %8s ' % sourceList[i], indent=1)
          if mosaic['otfUserlist'][i] == None:
              ...
              inputFile=utils.setMosaicFile(mosaic['otfUserlist'][i])
              commands.trackMessage('...using grid file %s' % inputFile, indent=2)
          mosaic['otfMosaic'].append(createOtfMosaic(mosaic, i))
       else:
...
def readOptions(var):
    ...
    p.add("lowcoh",   default=None, type=str,   description="Antennas where max coherence alarm should be ignored", noneAllowed=True)
...
def initializeTrack(sources,  mosaic,   projectInfo, limits,  
                    ...
                    restart=False, scriptName=None, scriptOptions=None, clearAb=True):
    ...
    if restart: 
        ...
        commands.radioInit(clearastrobands=clearAb)
    ...
    if restart: 
        if var['tune']: 
            commands.trackMessage('Setting tune to False for a track restart', indent=1)
        var['tune'] = False
    ...
    # Store max sensitivity variable. If more than one correlator is
    # in the subarray, then force maxsens to be True

    ownsSLWB = (commands.subarrayOwnsCorrelator(commands.subarrayNo, carma.util.CORR_SPECTRAL) and commands.subarrayOwnsCorrelator(commands.subarrayNo, carma.util.CORR_WIDEBAND));

    if ownsSLWB : var['maxsens'] = True

    if var['maxsens'] and not(ownsSLWB):
       raise Exception,'Maximum sensitivity is turned on, but the subarray doesnt currently own both the SPECTRAL and WIDEBAND correlators'
    ...
    # Save bad coherence antenna list
    lowcoh = var['lowcoh']
    if lowcoh == None: lowcoh = ''
    commands.s.setScriptString(INDX_STR_LOWCOH, lowcoh)
    ...
    # Check endtrack time
    if utils.timeRemaining(useTime=True) <= 0.0:
      raise Exception,'No time is remaining for track. Was the endtrack time set correctly?'
    ...
    if restart:
       ...
       if not var['maxsens'] and str(storedObsblock['subobsblock']).upper() != str(projectInfo['subobsblock']).upper():
       ...
       if obsblockName != commands.s.getScriptString(INDX_STR_OBSBLOCK_ID):
          if var['resetObsblock']:
             ...
             if runPacs or var['maxsens']:
                ...
                # For MAXSENS tracks, make sure subobsblock is 'SL' and 'WB'
                if var['maxsens']:
    ...
    tmpSubobsblock = ''
    if len(strial) == 4: tmpSubobsblock = strial[2]
    ...
    # Load user catalog
    if projectInfo['sourceCatalog'] not in [None, '']:
    ...
    # Check mosaic file
    if utils.isMosaic(sources['mosaicTarget']) and not mosaic['otf'] and mosaic['offsetFile'] <> None:
...
def needPointing(options, ref, intervalDefault, maxsep, force, verbose=True):
    """ Determine if we need to point.
        ...
        Input parameters:
            ...
            verbose         : If True, then print messages
    ...
    if not doRadioPoint and interval <> None and \
       commands.s.getScriptString(INDX_STR_LASTPOINT_NAME) != '' and ref <> None:
        ...
        distance = utils.getDistanceSky(ref, None, 
                           azo=commands.s.getScriptDouble(INDX_DBL_LASTPOINT_AZ),
                           elo=commands.s.getScriptDouble(INDX_DBL_LASTPOINT_EL))
        if distance > maxsep:
            doRadioPoint = True
            forceDistance = True
            msg = 'last pointing source was %.1f deg away ' % distance
            if verbose: commands.trackMessage(msg, indent=1)
    ...
    if not doRadioPoint and interval <> None and not inSameSunState and \
       ...
       utils.timeDifference(commands.lst(), commands.s.getScriptDouble(INDX_DBL_LASTPOINT), pos=True) >= interval:
        doRadioPoint = True
        msg = 'pointing %.1f hours' % dtSunRiseSet
    ...
    if not doRadioPoint and interval <> None and tpoint <> None:
        ...
        if dtNextPoint > 0.0: 
            if verbose: commands.trackMessage("pointing in " + utils.dtString(dtNextPoint), indent=1)
        elif dtEndOfTrack <> None and dtEndOfTrack > 0 and \
             dtEndOfTrack < 0.25 and interval > dtEndOfTrack:
            if verbose: commands.trackMessage("Skipping pointing since track will end in " + 
                         utils.dtString(dtEndOfTrack), indent=1)
        else:
            doRadioPoint = True
            if verbose: commands.trackMessage("pointing last done " + 
    ...
    # Do we need to force radio pointing?
...
def createPointingDictionary():
    ...
    pointing['pntwait'] = utils.getVariable(pointing,'pntwait', 2)
    pointing['waitCycles'] = utils.getVariable(pointing,'waitCycles', 3)
    pointing['noCameras'] = utils.getVariable(pointing,'noCameras', range(16,24))
...
def doPoint(options,             ref=None,
            ...
            checkSource=False,   verbose=True):
    """ Perform radio pointing on a source with an elevation greater than the
        ...
        Inputs: options     - pointing options; see template3.obs
                ...
                verbose       - If False, then do not print messages upto the checksource exit.
                                Radio point messages are still printed.
    ...
    # Message
    if printHeader and verbose: commands.trackMessage("Checking pointing")

    # Do we need pointing?
    doRadioPoint, doOptPoint, forceDistance, resetMountOffsets, sunStateCurrent = \
             needPointing(options, ref, interval, maxsep, force, verbose=verbose)
    ...
    if doRadioPoint:
        ...
        for allowShadowing in [False, True]:
           source = utils.getSource(sourceList, preferred=preferred, ref=ref, 
                      elmin=xelmin, elmax=xelmax, minsepsun=options['minsepsun'], 
    ...
    if doRadioPoint and source <> None:
        ...
        msg = 'pointing on ' + source + ' az=' + str('%.1f' % ae[0]) + '  el=' + str('%.1f' % ae[1])
        ...
        for i in range(options['nreps']):
            ...
            if doOptPoint:
                ...
                [optRes, badOptAnts, results] = \
                    ogm.deriveOptRadDiff(opticalSource, source,
                        min(commands.currentAntennaNumbers()), 
                        antVec=None, applyRadio=True, 
                        zoomVal=4.0, tune95=tune95Point,
                        antwait=options['antwait'], antennas=options['antennas'], 
                        timeLimit=options['timeLimit'], pntwait=options['pntwait'],
                        waitCycles=options['waitCycles'])

                # Save results in dictionary for later use
                options['optRes']     = optRes
                options['badOptAnts'] = badOptAnts
                options['noCameras'] = range(16,24)

                # Save results in script string for efficient restart
                badants = utils.list2string(options['badOptAnts'])
                nocameras = utils.list2string(options['noCameras'])
                sopt = ''
                for key in optRes:
                    if key not in badOptAnts:
                        sopt += ' ' + str(key) + ' ' + '%.4f' % optRes[key][0] + ' ' + '%.4f' % optRes[key][1]
                commands.s.setScriptString(INDX_STR_OPT_BADANTS, badants)
                commands.s.setScriptString(INDX_STR_OPT_NOCAMERAS, nocameras)
                ...
                results = radioPoint.radioPoint(source, type='triangle',
                        ...
                        waitCycles=options['waitCycles'], indent=jindent)
                print "Radio pointing is complete"

            # Print pointing results
            if dict not in [type(results)] :
                commands.trackMessage('WARNING: Radio pointing may have failed', indent=jindent)
            else:
                ...
                for i in ants:
                    ...
                    elif options['antennas'] is not None and i in options['antennas']:
                        msg += '     *** Not used for radio pointing     ***'
                    else:
                ...
                if doOptPoint:
                    ...
                    for i in commands.currentAntennaNumbers() :
                        if i not in options['noCameras'] and i not in options['badOptAnts'] : goodVals.append(i)
                    if len(goodVals) == 0 :
                        ...
                        for i in commands.currentAntennaNumbers():
                            ...
                            elif i in options['noCameras']:
                                msg += '      *** No optical cameras on antenna ***'
                            else:
            ...
            if doOptPoint:
                doOptPoint = False
                # Exclude antennas with no optical cameras from the count
                nbad = 0
                for i in options['badOptAnts']:
                    if i not in options['noCameras']: nbad += 1
                optPointOk = (nbad <= options['nbadAnts'])
                if optPointOk:
...
def updatePointingOptical(pointing, source, sunStateCurrent, indent=1) :
    ...
    for i in commands.currentAntennaNumbers():
        ...
        elif i in pointing['noCameras']:
            commands.trackMessage('  %2d       %16s' % (i,"*** No optical camera on antenna ***"), indent=indent)
        else:
...
def doPassband(options,          noise, 
               ...
               start=False,      email=None,     lastchance=True):
    """ Observe a passband calibrator with an elevation greater than the
        ...
        Inputs: options     - passband options; see template3.obs
                ...
                start       - If True, this is the initial passband observation
                              for the track. This is used in 
                              conjunction with options['pbstart']. If start=False,
                              then it is assumed this is the last chance to
                              observe a passband calibrator.
                printHeader - Print message with indent=0
                ...
                lastchance  - If True and start=False, then observe the brighest passband
                              calibrator available since the script is ending.
                              Ideally lastchance should not depend on start,
                              but this was done to maintain backwards capatability.
    ...
    passbandCal = utils.getSource(sourceList, preferred=options['preferred'], ref=ref,
                     ...
                     diameterFraction=options['diameterFraction'],
                     allowShadowingPreferred=options['allowShadowingPreferred'])
    ...
    # If no source was found, but this is the last change to observe
    # a passband source, then pick any source
    isLastchance = False
    fluxLastchance = None
    if passbandCal == None and not start and lastchance:
       # Get sources in decreaase order of fluxes
       sourceDict  = utils.getBrightSources(0.5, freq=lofreq, returnFluxes=True)
       sourcesSorted = sorted(sourceDict.items(), key=lambda x:-x[1])

       # Find brightest source that is ok to observe
       for s in sourcesSorted:
          passbandCal = utils.getSource([s[0]], preferred=options['preferred'], 
                          elmin=options['elmin'], elmax=options['elmax'],
                          excludeExpref=excludeExpref, minsepsun=options['minsepsun'],
                          timeAvailable=options['tint'], allowShadowing=False,
                          diameterFraction=options['diameterFraction'])
          if passbandCal != None: break

       # Print message
       if passbandCal != None:
          passbandCal = passbandCal.upper()
          isLastchance = True
          fluxLastchance = sourceDict[passbandCal]
          commands.trackMessage("No passband calibrator available - observing %s (%.1f Jy) as a last resort" % (passbandCal, sourceDict[passbandCal]), indent=1)
    ...
    # Set integration time. If this is the "lastchance" source, then
    # increase integration time a bit
    tint = options['tint']
    if isLastchance and fluxLastchance < options['minflux']:
       tint *= (options['minflux'] / fluxLastchance)**2
       tint = min(tint, max(15.0, options['tint']) )
...
#     utils.sendEmailMessage('obs@mmarray.org', 'obs@mmarray.org', 'The passband calibrator is about to be observed.\nPlease check the gains page at\n   http://cedarflat.mmarray.org/gains/index_sci%d.html\nto verify that all of the antennas have coherence.' % commands.subarrayNo, subject='Got coherence?')
    ok = observeSource(passbandCal, tint, options['record'],
    ...
    for i in range(len(phase)):
        ...
        if action == "BANDPASS" or action == "PASSBAND":
            if not finishedPassband(passband):
                doPassband(passband, noise, pointing=pointing, ref=srefCal, lastchance=False)
            ...
            doSnapshot(sources, pname, pointing, passband, 
                  fluxcal, noise, mosaic, primaryCal=primaryCal, 
...
def doCycles(sources,         phaseCal,     pointing,     passband, 
    ...
    # Set LST start/stop time for targets
    lststartTarget = [None] * len(sou)
    lststopTarget  = [None] * len(sou)
    if sources.has_key('lststartTarget'):
       junk,lststartTarget = utils.setSourceVariable(sources['target'], sources['lststartTarget'], None)
    if sources.has_key('lststopTarget'):
       junk,lststopTarget = utils.setSourceVariable(sources['target'], sources['lststopTarget'], None)
    ...
    while continueCycles :
        ...
        if (not commands.s.getScriptBool(INDX_BOOL_OPTRAD_VECTOR) or \
           ...
           utils.continueObservations(isScience=True) and \
           utils.timeRemaining(useTime=True, lsttime=utils.getLstStopCycle(default=pstop)) > 0.0:
            pointed = doPoint(pointing, ref=srefCal, printHeader=False)
        ...
        if not finishedPassband(passband) and passband['middle'] and \
           ...
           utils.timeRemaining(useTime=True, lsttime=utils.getLstStopCycle(default=pstop)) > 0.0:
            observedPassband = doPassband(passband, noise,
                ...
                printHeader=False, lastchance=False)
           ...
           utils.continueObservations(isScience=True) and tremaining > 0.0:
            ...
            for i in range(len(sou)):
                ...
                tremaining = utils.timeRemaining(source=sou[i], phase=phaseCal,
                     ...
                     tneed=tneed)
                if lststartTarget[i] <> None:
                   dt = utils.timeDifference(commands.lst(), lststartTarget[i])
                   if dt < 0.0: continue
                if lststopTarget[i] <> None:
                   xt = utils.timeRemaining(source=sou[i], phase=phaseCal,
                     useTime=True, 
                     lsttime=utils.getLstStopCycle(default=lststopTarget[i]),
                     tneed=tneed)
                   tremaining = min(tremaining, xt)

                # Observe source
                inttime = 0.0
                if mosaic_source:
                    ok, xt, junk = observeMosaic(sou[i], t[i], mosaic, sources,
...
def runGainCheck(subNo, miriadFile, refant, runPacs=False):
   """ Runs the gain check script for miriadFile.

       Example:
           rungainCheck(1, '/opt/sdp/sciencedata/c1045.1E_110HLtau.1.mir', 7)
   """

   com = '/usr/bin/lockrun --lockfile=/array/obs/bin/lockrun.sci%d -- /array/obs/bin/gainCheckSci%d.csh ' % (subNo, subNo) + \
     'vis=' + miriadFile + ' ' + \
     'subarray=' + str(subNo)
   if refant is not None: com += ' refant=' + str(refant)
   if runPacs:
   ...
   m = "Submitting gains script in the background with: " + com
   commands.commandlog(m)
   tt = None
   os.system("%s >& /dev/null &" % com)
#  tt = os.popen3(com)
   # The close statements stop the script, so they are commented out
   #tt[0].close()
   #tt[1].close()
   #tt[2].close()
   return tt
...
def observeSource(sources,          tint,                trecordDefault,
                  ...
                  noise=None,       tsysdict=None, azWrapMode=commands.TRACKTIME):
    """ Observe "sources" for integration time "tint" minutes with a
       ...
       Inputs: sources    - string or string list of "sources" to observe
               ...
               azWrapMode  - override the default tracking wrap mode.  In general, don't use
                             this option as it is implemented mainly to fix problems with Karto's script. 
    ...
    for isource in range(len(sou)):
        ...
        if maximumTime <> None: 
            tremaining = min(tremaining, (maximumTime - utils.timeDifference(commands.lst(),tstart,pos=True)))
        ...
        if not isUp:
            ...
            if not isNoiseSource: 
                ...
                # Noise source
                if noise <> None:
                    commands.track(sou[isource],azWrapMode=azWrapMode)
            ...
            commands.scriptlog('DEBUG2: nrepsList=%s'% str(nrepsList))
            for j in range(len(nrepsList)):
                # Check nreps - debugging
                n = nrepsList[j]
                commands.scriptlog('DEBUG2: tremaining=%s' % str(tremaining))
                commands.scriptlog('DEBUG2: integrationTime=%s' % str(integrationTime))
                commands.scriptlog('DEBUG2: maximumTime=%s' % str(maximumTime))
                commands.scriptlog('DEBUG2: elmin=%s' % str(elmin))
                commands.scriptlog('DEBUG2: lststop=%s' % str(lststop))

                # Stop if ENDTRACK or LASTCAL is set
                if utils.isEndOfTrack() or (utils.isLastcal() and isScience): 
                   commands.scriptlog('DEBUG3: stopping in observeSource')
                   commands.scriptlog('DEBUG3: isEndOfTrack='+str(utils.isEndOfTrack()))
                   commands.scriptlog('DEBUG3: isLastcal='+str(utils.isLastcal()))
                   commands.scriptlog('DEBUG3: isScience='+str(isScience))
                   break

                # Process number of reps
                if n > 0 and utils.continueObservations(isScience=isScience):
                    ...
                    measureTsys = True
                    if tsysTime == 0 or isNoiseSource:
                        measureTsys = False
                    ...
                    if isNoiseSource:
                        ...
                        if not isNoiseSource:
                            # Check coherence
                            if intents[isource].upper() in ['B', 'G', 'F']:
                               utils.checkCoherence(indent=indent)

                            # Turn on mox coherence alarm, if nedded
                            if not commands.s.getScriptBool(INDX_BOOL_ENABLED_MAXCOH):
                               commands.s.setScriptBool(INDX_BOOL_ENABLED_MAXCOH, 1)
            ...
            if runMiriad and len(nrepsList) > 0:
                ...
                # Run gain check commands
                dataDirectory = '/opt/sdp/sciencedata/'
                if commands.s.getScriptBool(INDX_BOOL_MAXSENS):
                   miriadFile = dataDirectory + commands.queryString('Control.SpectralLineCorrelator.obsBlockId') + '.mir'
                   tt1 = runGainCheck(1, miriadFile, None, runPacs=runPacs)
                   miriadFile = dataDirectory + commands.queryString('Control.WidebandCorrelator.obsBlockId') + '.mir'
                   tt2 = runGainCheck(2, miriadFile, None, runPacs=runPacs)
                else:
                   miriadFile = dataDirectory + utils.getObsblockName() + '.mir'
                   tt1 = runGainCheck(commands.subarrayNo, miriadFile, refAnt, runPacs=runPacs)
...
def observeMosaic(souMosaic, integTimeMosaic, mosaic, sources, lststop=None, 
    ...
    """ Make mosaic
        ...
        Inputs: souMosaic     - source to observe
                ...
                nposMax     - Maximum number of positions to observe in the mosaic
    ...
    # Keep track of number of positions observed
    nposObserved = 0

    # Set mosaic offsets or initialize OTF mosaic. 
    # Offsets are stored in arcminutes within mosaic['offsetsArcmin'] keyword
    if not mosaic['otf'] and (not mosaic.has_key('offsetsArcmin') or mosaic['offsetsArcmin'] is None):
        # Read offsets
    ...
    if not mosaic['otf']:
       ...
       if nposMax is not None: nposCycle = min([nposCycle, nposMax])
    ...
    if utils.timeRemaining(source=souMosaic, lsttime=lststop) > 0:
        ...
        tintTotalPosition = 0 # Total integration time at a position; used only for pointed mosaics
        while observedSourceInCycle and utils.continueObservations(isScience=True):
            ...
            xtremain = utils.timeRemaining(source=souMosaic, lsttime=lststop)
            xlst = commands.lst()
            tintMosaic = min(integTimeMosaic-tintTotalPosition, xtremain * 60)
            if maximumTime <> None: 
                tintMosaic = min(tintMosaic, (maximumTime - utils.timeDifference(xlst,tstart,pos=True)) * 60.0)
            if tintMosaic < utils.minimumIntegrationTime() / 60.0: 
               commands.scriptlog('DEBUG observeMosaic 1: tintMosaic = %s' % str(tintMosaic))
               commands.scriptlog('DEBUG observeMosaic 1: integTimeMosaic = %s' % str(integTimeMosaic))
               commands.scriptlog('DEBUG observeMosaic 1: tintTotalPosition = %s' % str(tintTotalPosition))
               commands.scriptlog('DEBUG observeMosaic 1: maximumTime = %s' % str(maximumTime))
               commands.scriptlog('DEBUG observeMosaic 1: lst         = %s' % str(xlst))
               commands.scriptlog('DEBUG observeMosaic 1: tremaining  = %s' % str(xtremain))
               continue
            ...
            if mosaic['otf']: 
               ...
               commands.scriptlog('DEBUG observeMosaic 2: nposObserved = %d' % nposObserved)
               commands.scriptlog('DEBUG observeMosaic 2: nposCycle    = %d' % nposCycle)
               continue
            ...
            mintint = 4.0/60.0
#           if mosaic['otf']: mintint = 2.0*mosaic['otfFieldtime'][index]/60.0
            if mosaic['otf']: mintint = 1.0
            if integTimeMosaic <= 0.5 and tint < integTimeMosaic: 
               tint = integTimeMosaic
            elif integTimeMosaic > mintint and tint < mintint: 
               tint = mintint

            # Make the mosaic
            if mosaic['otf']:
               # There appears to be some bug in OTF where if maxtint is too small, then
               # it ignores maxtint. I suspect it is an integer truncation problem.
               # So here, I impose a minimum integration time of 1 minute
               tint = max(tint, 1.0)
               ...
               # Integrate
               nposStart = mosaic['otfMosaic'][index].fieldIndex
               mosaic['otfMosaic'][index].takeData(maxtime=tint)
               nposStop  = mosaic['otfMosaic'][index].fieldIndex - 1

               # Set actual integration time based on number of positions observed
               tint = (nposStop - nposStart + 1) / 60.0 * \
                       (mosaic['otfMosaic'][index].fieldtime + mosaic['otfMosaic'][index].slewtime)

               # Update counters
               observedSource = True
               observedSourceInCycle = True
               nposObserved += (nposStop - nposStart + 1)
               utils.setLastMosaicPosition(nposStop)
            else:
               # Set mosaic position
               ...
               if nreps > 0:
                   ...
                   tintTotalPosition += tint
                   if tint >= 0:
                       observedSource = True
                       observedSourceInCycle = True
                   if tintTotalPosition >= integTimeMosaic:
    ...
    return observedSource, integrationTimeTotal, nposObserved


def getSnapshotDatabase(isMosaic, sources):
    """ Gets the snapshot database """

    # Set database
    if isMosaic:
       db = sn.DB_MOSAIC_DEFAULT
       if sources.has_key('snapDbMosaic'): 
          db = sources['snapDbMosaic']
    else:
       db = sn.DB_DEFAULT
       if sources.has_key('snapDb'): 
          db = sources['snapDb']
...
def checkSnapshotSources(sources, mosaic, exit=True):
    ...
    # Loop over snapshot projects
    targets = utils.makeListObsdef(sources['target'])
    for i in range(len(targets)):
        # Get database for this project
        db = getSnapshotDatabase(sources['mosaicTarget'][i], sources)

        # First, make sure snap project code exists
        com = "select count(*) from projects where lower(project) = '" + sources["snapProject"][i].lower() + "'"
        result = sn.execdb(com, db=db)
        n = int(result[0][0])
        if n == 0:
             raise Exception,'The snapshot code %s was not found in the snapshot database %s.\n Check the keyword sources["snapProject"] in the sources dictionary.' % (sources['snapProject'][i], str(db))
        elif n > 1:
             raise Exception,'The snapshot code %s was found multiple times in the snapshot database %s.\n Something is seriously wrong!' % (sources['snapProject'][i], str(db))

        # Project code exists, so now check that sources are available.
        if sources['mosaicTarget'][i]:
          com = "select count(*) from sources s,projects p " + \
              "where s.maxindex - s.lastobservedindex > 0 and s.maxindex > 0"
        else:
          com = "select count(*) from sources s,projects p where s.treq - s.tobs > 0"
        com += " and s.pid=p.pid and lower(project) = '" + sources["snapProject"][i].lower() + "'" 
        result = sn.execdb(com, db=db)
        ntot += int(result[0][0])

    # Any sources?
    if ntot == 0:
       raise Exception,'No sources were found in the snapshot database'


def getNextSource(sources, tneed, pstop, phaseCalDefault, db=None, 
                  isMosaic=False, snapProject=None, sidforce=None, noComplete=False):
    """ Returns the next source in the project 
        ...
        Inputs
            ...
            db     :  Name of the database containing the information
            isMosaic : If True, then this is a mosaic source
    ...
    # Set database
    if db == None:
       db = getSnapshotDatabase(isMosaic, sources)

    # Set snapshot project
    sp = snapProject
    if sp == None: sp = sources['snapProject'].lower()

    # Execute database command to find next sources
    com = "select s.sid,s.name,s.phasecal,s.secondcal"
    if isMosaic:
       com += ",s.hastart,s.hastop,s.lastobservedindex,s.isotf,s.nphase,s.arcminunits,s.offsetsfile,s.pnttint,s.otfdelta,s.otfpa,s.otfrows,s.otffieldsinrow,s.otfrowoffset,s.otfcoloffset,s.otffieldtime,s.offdalp,s.offddec,s.tintoff,s.intervaloff,s.nrepeats,s.nmosaic,s.maxindex" + \
             " from sources s,projects p " + \
             " where s.maxindex-s.lastobservedindex > 0 and s.maxindex>0"
    else:
       com += ",s.treq-s.tobs as dt" + \
             " from sources s,projects p " + \
             " where s.treq-s.tobs > 0.0"
    if sidforce is not None:
       com += " and s.sid=%d" % sidforce
    com += " and s.pid=p.pid and lower(project) = lower('" + sp + "') order by s.priority,s.sid"
    print com
    result = sn.execdb(com, db=db)
    ...
    for key, value in result.items():
        ...
        secondaryCal = value[3]
        phaseCal = phaseCalDefault
        if primaryCalibrator != "": phaseCal = primaryCalibrator
        if not isMosaic:
            tallocated = float(value[4]) / 60.0 # convert from minutes to hours
        ...
        tremaining = utils.timeRemaining(source=source, phase=phaseCal, 
                       ...
                       lsttime=utils.getLstStopCycle(default=pstop))
        if tremaining <= 0.0: 
            if sources['snapVerboseSelection']: print source, ': no time allocated or not up'
            continue
        ...
        if el < utils.getElevationLimit(): 
            if sources['snapVerboseSelection']: print source, ': below system elevation limit', el, utils.getElevationLimit()
            continue
        if elcal <> None and elcal < utils.getElevationLimit(): 
            if sources['snapVerboseSelection']: print source, ': cal', phaseCal, 'below system elevation limit', elcal, utils.getElevationLimit()
            continue

        # Skip source if it is above users elevation limit
        if sources['snapElmax'] <> None:
            if el    > sources['snapElmax']: 
                if sources['snapVerboseSelection']: print source, ': above maximum elevation', el, sources['snapElmax']
                continue
            if elcal <> None and elcal > sources['snapElmax']: 
                if sources['snapVerboseSelection']: print source, ':', phaseCal, 'above maximum elevation', elcal, sources['snapElmax']
                continue

        # Check user-specified elevation ranges
        ha = utils.getHa(source)
        #hacal = None
        #if phaseCal <> None: hacal = utils.getHa(phaseCal)
        if sources['snapElHa'] <> None:
            # AND or OR for el/ha?
            isOR = True
            if len(sources['snapElHa']) == 4 and \
               sources['snapElHa'][3] not in [False, 0, None]:
               isOR = False

            # Evaluate
            ok = False
            if isOR:
               if (el >= sources['snapElHa'][0]) or \
                  (ha >= sources['snapElHa'][1] and ha <= sources['snapElHa'][2]):
                   ok = True
            elif (el >= sources['snapElHa'][0]) and \
                  (ha >= sources['snapElHa'][1] and ha <= sources['snapElHa'][2]):
                   ok = True
            if not ok: 
                if sources['snapVerboseSelection']: print source, ': fails standard minimum elevation/hour angle range test', el, ha, sources['snapElHa']
                continue

        # If a minimum elevation for snapshot observations is set, then
        # see how much longer the source and phasecal will be above it.
        # Require source+cal to be up for at least TimeBeforeSet longer, the idea being that this should be ~1 cycle.
        # snapPreferComplete has precedence on the first pass. If no acceptable targets are found, the preference for
        # completing a target is ignored on the second pass in favor of doing at least something.
        # If snapRequireComplete, there is no second pass.
        # This is NOT compatible with mosiac mode.
        if sources['snapElHa'] <> None and sources['snapTimeBeforeSet'] <> None:
            elMin = sources['snapElHa'][0]
            # minimum of time remaining in track and time source+phasecal are both above elMin
            tmax = utils.timeRemaining(source=source, phase=phaseCal, 
                                       useTime=True, tneed=tneed,
                                       lsttime=utils.getLstStopCycle(default=pstop),
                                       elmin=elMin)
            if (sources['snapPreferComplete'] or sources['snapRequireComplete']) and not noComplete:
                treq = tallocated
            else:
                treq = min(sources['snapTimeBeforeSet'], tallocated)
            if tmax < treq: 
                if sources['snapVerboseSelection']: print source, ': setting soon', tmax, sources['snapTimeBeforeSet'], treq, tallocated, tmax >= treq
                continue

        # do the same checks for the maximum elevation
        if sources['snapElmax'] <> None and sources['snapTimeBeforeSet'] <> None:
            elMin = sources['snapElmax']
            tsource = utils.getTimeToRise(source, elmin=elMin)
            if tsource == 0.0: tsource = 100.0 # getTimeToRise returns 0 if the source never exceeds elmin
            tphase = utils.getTimeToRise(phaseCal, elmin=elMin)
            if tphase == 0.0: tphase = 100.0
            tmax = min(tsource, tphase, tremaining)
            if (sources['snapPreferComplete'] or sources['snapRequireComplete']) and not noComplete:
                treq = tallocated
            else:
                treq = min(sources['snapTimeBeforeSet'], tallocated)
            if tmax < treq: 
                if sources['snapVerboseSelection']: print source, ': source or cal will pass through zenith soon', tmax, sources['snapTimeBeforeSet'], treq, tallocated, tmax >= treq
                continue

        # Special checks for mosaic
        if isMosaic:
           # Check hour angle
           hamin = float(value[4])
           hamax = float(value[5])
           if ha < hamin or ha > hamax: 
               if sources['snapVerboseSelection']: print source, ': fails special mosiac hour angle range test'
               continue

        # OK - we have a source
        if sources['snapVerboseSelection']: print "We have a source:", source
        prop = dict()
        prop['sid'] = int(value[0])
        prop['target'] = source
        prop['phasecal'] = phaseCal
        prop['secondarycal'] = secondaryCal
        if isMosaic:
           prop['hastart']            = float(value[4])
           prop['hastop']             = float(value[5])
           prop['lastobservedindex']  = int(value[6])
           prop['isotf']              = int(value[7])
           prop['nrepeats']           = int(value[23])
           prop['maxindex']           = int(value[25])
           if prop['isotf']:
              prop['otfdelta']           = float(value[12])
              prop['otfpa']              = float(value[13])
              prop['otfrows']            = int(value[14])
              prop['otffieldsinrow']     = int(value[15])
              prop['otfrowoffset']       = float(value[16])
              prop['otfcoloffset']       = float(value[17])
              prop['otffieldtime']       = float(value[18])
           else:
              prop['pntnphase']          = int(value[8])
              prop['pntarcminunits']     = int(value[9])
              prop['offsetsfile']        = value[10]
              prop['pnttint']            = float(value[11])
              if value[24] == '':
                 prop['nmosaic'] = 1
              else:
                 prop['nmosaic'] = int(value[24])
           if value[19] != '' and value[20] != '':
              prop['offdalp']            = float(value[19])
              prop['offddec']            = float(value[20])
              prop['offtint']            = float(value[21])
              prop['offinterval']        = float(value[22])
           else:
              prop['offdalp'] = None
              prop['offddec'] = None
              prop['offtint'] = 0
           ...
           prop['tremaining'] = float(value[4])

        # Found a source - break loop
        break

    # Return source
    if sources['snapVerboseSelection']: print prop
    return prop


def findPhaseCalibrator(sources, tuneIndx, snapFreq, sou_snap, phaseCalPrevious):
    """ Finds a phase calibrator snapshot observing scripts """
    ...
    phaseCal = utils.getSource(sourceList, ref=sou_snap, allowShadowing=True,
    ...
    # Done
    return phaseCal, tintPhaseCal


def doSnapshot(sources, phaseCalDefault, pointing, passband, fluxcal, 
             noise, mosaic, primaryCal=None, pstop=None, resetDoppler=False):
    """ Perform one cycle between phaseCal and sources.
        ...
        Inputs: sources    - contains science targets and phase calibrators;
                ...
                fluxcal    - flux calibrator options; see template3.obs
                noise      - noise source options; see template3.obs
                mosaic     - mosaic dictioanry; see template3.obs
                passband   - passband options; see template3.obs
    ...
    """
    # Set database name
    dbSingle = sn.DB_DEFAULT
    dbMosaic = sn.DB_MOSAIC_DEFAULT
    if sources.has_key('snapDb'): db = sources['snapDb']
    if sources.has_key('snapDbMosaic'): db = sources['snapDbMosaic']
    ...
    # Get source targets and integration times
    targets, tintTargets = utils.setSourceVariable(sources['target'], sources['tintTarget'], 0)
    intents = utils.setSourceVariable(sources['target'], sources['intentTarget'], 'S')[1]
    pointstatus = utils.setSourceVariable(sources['target'], sources['pointstatusTarget'], commands.ONSRC)[1]

    # Compute time between phase calibrator observations
    tintBetweenPhasecal = sum(tintTargets)
    if sources.has_key('snapTintBetweenPhasecal'): tintBetweenPhasecal = sources['snapTintBetweenPhasecal']
    ...
    tsecondaryCal = sources['snapTintSecondarycal']
    ...
    while continueCycles :
        # Set number of frequency loops to make. This is nominally 1, but 
        # can be more than once for a multifrequency track. 
        nfreq = 1
        ...
        for i in range(len(targets)):
            ...
            # Find which source to observe next. If a source ID is stored and snapFollow=True,
            # Then check the source ID name
            okCal = False
            db = dbSingle
            result = None
            if sources['mosaicTarget'][i]: db = dbMosaic
            if sources['snapFollow'] and commands.s.getScriptBool(INDX_BOOL_SNAPID):
               result = getNextSource(sources, tneed, pstop, phaseCalDefault, 
                                      db=db, isMosaic=sources['mosaicTarget'][i], 
                                      snapProject=sources['snapProject'][i], 
                                      sidforce=commands.s.getScriptInt(INDX_INT_SNAPID))
            if result == None:
               result = getNextSource(sources, tneed, pstop, phaseCalDefault, 
                                      db=db, isMosaic=sources['mosaicTarget'][i], 
                                      snapProject=sources['snapProject'][i])
            if result is None and sources['snapPreferComplete'] and not sources['snapRequireComplete']:
               result = getNextSource(sources, tneed, pstop, phaseCalDefault, 
                                      db=db, isMosaic=sources['mosaicTarget'][i], 
                                      snapProject=sources['snapProject'][i], 
                                      noComplete=True)

            # If no snapshot sources remain, then skip
            if result == None:
                commands.trackMessage('No valid snapshot sources found in database %s, project %s' % 
                                      (db, sources['snapProject'][i]))
                continueCycles = False
                continue
            ...
            phaseCal = None
            secondaryCal = None
            sou_snap = result['target']
            if phaseCalDefault <> None: phaseCal = phaseCalDefault.strip()
            if result['phasecal'] is not None and result['phasecal'] != "": 
                phaseCal = result['phasecal'].strip()
            if result['secondarycal'] is not None and result['secondarycal'] != "": 
                secondaryCal = result['secondarycal'].strip()
            sid = result['sid']

            # If finding calibrators automatically, then set the phase cal for the first 
            # tuning. This is needed to see if we need to observe the phase calinrator 
            # before radio pointing.
            tintPhaseCal = sources['tintPhaseCal']
            phaseCalOrig = phaseCal
            if phaseCal == None and autoPhase[0]:
                phaseCal, tintPhaseCal = findPhaseCalibrator(sources, 0, 
                                              snapFreq, sou_snap, phaseCalPrevious)
            ...
            # Check if we need to observe the PREVIOUS phase calibrator one last time because:
            #     1) phase calibrator has changed
            #     2) we need to retune
            ...
            if okSource and utils.continueObservations() and not isMultiFreq and \
               ( (phaseCalPrevious <> None and phaseCal <> None and phaseCal != phaseCalPrevious) or
                ...
                okCal = observeSource(phaseCalPrevious, tintPhaseCal,
                             sources['record'], tsysTime=sources['tsys'], 
            ...
            # For optical pointing, there can be different results depending on 
            # whether optical pointing is done only on the phase calibrator, or 
            # whether it is done on the phase calibrator and target source.
            ...
            if commands.s.getScriptBool(INDX_BOOL_OPTRAD_VECTOR) and \
               ...
               utils.continueObservations(isScience=True) and \
               utils.timeRemaining(useTime=True, lsttime=utils.getLstStopCycle(default=pstop)) > 0.0:
                resultPoint = doPoint(pointing, ref=srefCal, printHeader=False)
            ...
            for indx in range(nfreq):
                 ...
                 tintPhaseCal = sources['tintPhaseCal']
                 if phaseCalOrig == None and autoPhase[tuneIndx]:
                     phaseCal, tintPhaseCal = findPhaseCalibrator(sources, tuneIndx, snapFreq, sou_snap, phaseCalPrevious)
                 ...
                 if indx == 0 and not performingOpticalPointing:
                     needRadioPointing = needPointing(pointing, sou_snap, None, pointing['maxsep'], False, verbose=False)[0]
                     if needRadioPointing and not doPoint(pointing, ref=sou_snap, printHeader=False, checkSource=True, verbose=False):
                        needRadioPointing = False
                        commands.trackMessage('no radio pointing source is available', indent=1)

                 # Observe phase calibrator, if needed because:
                 # 1) We need radio pointing and we must bracket the phase calibrator before and after pointing
                 # 2) The phase calibrator is about to change
                 # 3) Too much time has elapsed since last phase cal observation
                 # 4) 
                 okCal = False
                 if phaseCal == None:
                     okCal = True
                 elif utils.continueObservations() and tremaining > 0.0:
                     # Check if we need to observe the CURRENT phase calibrator
                     observePhaseCal = (needRadioPointing and okSource) or \
                                   (not startingCycles and not needRadioPointing and phaseCalPrevious != phaseCal) or \
                                   (not isMultiFreq and tintSinceLastPhaseCal >= tintBetweenPhasecal) or \
                 ...
                 # Check calibrators, but only if we are not in the middle of using a calibrator
                 if tintSinceLastPhaseCal == 0:
                     # Flux calibrator
                     observedFluxcal = False
                     if not finishedFluxcal(fluxcal) and fluxcal['middle'] and \
                        utils.continueObservations(isScience=True) and \
                     ...
                     if not finishedPassband(passband) and passband['middle'] and \
                        utils.continueObservations(isScience=True) and \
                        utils.timeRemaining(useTime=True, lsttime=utils.getLstStopCycle(default=pstop)) > 0.0:
                         observedPassband = doPassband(passband, noise, lastchance=False,
                               pointing=pointing, ref=sou_snap, lststop=pstop, printHeader=False)

                 # It is possible at this point that we have not 
                 #   a) pointed,
                 #   b) observed a flux calibrator
                 #   c) observed a passband calibrator
                 ...
                 xt = 0 # Integrate time for the source observations
                 if not pointed and not observedFluxcal and not observedPassband and \
                    utils.continueObservations(isScience=True) and tremaining > 0.0:
                     # Mark tuning as used
                     ...
                     if commands.s.getScriptBool(INDX_BOOL_OPTRAD_VECTOR) and \
                        commands.s.getScriptBool(INDX_BOOL_DO_OPT_POINT) and \
                        pointing['doOptPoint'] and pointing['optPointTarget']:
                         doPoint(pointing, ref=srefTarget, printHeader=False)
                     ...
                     # Set integration time
                     if sources['mosaicTarget'][i]:
                         tz = min(tintBetweenPhasecal - tintSinceLastPhaseCal, 
                                  tintTargets[i])
                     else:
                         tz = min(result['tremaining'],
                                  tintBetweenPhasecal - tintSinceLastPhaseCal, 
                                  tintTargets[i])

                     # Set mosaic parameters and integration time.
                     # For pointed mosaics, tint is the integration time per position. This
                     # will be taken from the database.
                     # For OTF mosaics, this is the total time to spend on the observations.
                     # This is given in the sources['tintTarget'] field.
                     t1 = time.time()
                     tint = None
                     elStart = utils.getSourceElevation(sou_snap)
                     startingIndex = None
                     if sources['mosaicTarget'][i]:
                        # Maximum number of positions to observed. Used in pointed mosaics only
                        nposMax = None

                        # Set mosaic parameters
                        if result['isotf']:
                           mosaic['otf'] = True
                           mosaic['otfFieldtime'] = result['otffieldtime']
                           mosaic['otfRows'] = result['otfrows']
                           mosaic['otfFieldsInRow'] = result['otffieldsinrow']
                           mosaic['otfFieldtime'] = result['otffieldtime']
                           mosaic['otfDelta'] = result['otfdelta']
                           mosaic['otfPA'] = result['otfpa']
                           mosaic['otfRowOffset'] = result['otfrowoffset']
                           mosaic['otfColOffset'] = result['otfcoloffset']
                           mosaic['otfMaxreps'] = result['nrepeats']
                           if result['lastobservedindex'] < 0:
                              mosaic['otfStartFieldIndex'] = 0
                              utils.setLastMosaicPosition(-1)
                              startingIndex = 0
                           else:
                              mosaic['otfStartFieldIndex'] = result['lastobservedindex'] + 1
                              utils.setLastMosaicPosition(result['lastobservedindex'])
                              startingIndex = result['lastobservedindex'] + 1
                           tint = tintTargets[i]

                           # Create OTF mosaic
                           setMosaicParameters(mosaic, sources)
                           mosaic['otfMosaic'][i] = createOtfMosaic(mosaic, i)
                        else:
                           # Set mosaic parameters
                           mosaic['otf'] = False
                           mosaic['startpos'] = result['lastobservedindex']
                           mosaic['nphase'] = result['pntnphase']
                           mosaic['arcminUnits'] = result['pntarcminunits']
                           mosaic['offsetFile'] = result['offsetsfile']
                           mosaic['offsets'] = None
                           mosaic['offsetsArcmin'] = None
                           tint = result['pnttint']
                           if result['lastobservedindex'] < 0:
                              utils.setLastMosaicPosition(-1)
                              startingIndex = 0
                              nposMax = result['maxindex']
                           else:
                              utils.setLastMosaicPosition(result['lastobservedindex'] % result['nmosaic'] + 1)
                              startingIndex = result['lastobservedindex'] + 1
                              nposMax = result['maxindex'] -  result['lastobservedindex']
 
                        # Set off position
                        if result['offdalp'] != "" and result['offdalp'] is not None:
                           mosaic['off'][i] = [float(result['offdalp']), float(result['offddec'])]
                        mosaic['tintOff'][i] = result['offtint']
                        mosaic['intervalOff'][i] = result['offinterval']

                        # Observe mosaic
                        ok, xt, np = observeMosaic(sou_snap, tz, mosaic, sources,
                               maximumTime=tremaining, 
                               lststop=utils.getLstStopCycle(default=pstop),
                               noise=noise,
                               intent=intents[i],
                               pointstatus=pointstatus[i],
                               tsysdict=tsysdict, index=i, nposMax=nposMax)
                        nposObserved += np
                        xt *= 60.0 # Convert to seconds
                     else:
                        xt = observeSource(sou_snap, tz, sources['record'],
                            isScience=True, lststop=utils.getLstStopCycle(default=pstop),
                            maximumTime=tremaining, 
                            tsysTime=sources['tsys'], 
                            tmo=sources['tmo'], resetDoppler=resetDoppler,
                            antwait=sources['antwait'], intentObject=intents[i],
                            selfcal=False, tsysdict=tsysdict, returnTint=True)
                     elStop = utils.getSourceElevation(sou_snap)
                     elSouSnap = 0.5 * (elStart + elStop)

                     # Adjust time remaining for integration time on source
                     dt = time.time() - t1
                     tremaining -= dt / 3600.0
                     tint_actual_target += xt
                     tint_actual_cycle  += xt
                     tintSinceLastPhaseCal += xt/60.0

                     # Observe secondary calibrator 
                     if secondaryCal <> None and xt > 0:
                         xst = observeSource(secondaryCal, tsecondaryCal, sources['record'],
                               isScience=True, lststop=utils.getLstStopCycle(default=pstop),
                               tsysTime=sources['tsys'], maximumTime=tremaining, 
                               tmo=sources['tmo'], resetDoppler=resetDoppler,
                               antwait=sources['antwait'], intentObject='S', 
                               selfcal=False, returnTint=True)
                         tintSinceLastPhaseCal += xst/60.0

                     # For multifrequency observations, end on the calibrator
                     if isMultiFreq and xt > 0 and phaseCal <> None:
                         xst = observeSource(phaseCal, tintPhaseCal,
                               sources['record'], tsysTime=sources['tsys'], 
                               noise=noise,
                               lststop=utils.getLstStopCycle(default=pstop),
                               tmo=sources['tmo'], antwait=sources['antwait'],
            ...
            # Get delta time and increment database after all frequency observations are completed
            commands.s.setScriptBool(INDX_BOOL_SNAPID, 0)
            if tint_actual_target > 0:
                okSource = True
                utils.addSourceTime(tint_actual_target/3600.0)
                weather = utils.getWeather()
                isUseable = (elSouSnap >= sources['snapElMinUseable'] and elSouSnap <= sources['snapElMaxUseable'])
                commands.s.setScriptBool(INDX_BOOL_SNAPID, 1)
                commands.s.setScriptInt(INDX_INT_SNAPID, sid)
                if sources['mosaicTarget'][i]:
                   endingIndex = startingIndex  + nposObserved - 1
                   sn.addMosaicDatabase(sid, utstart, 
                          startingIndex, endingIndex, nposObserved,
                          len(commands.currentAntennaNumbers()), 
                          utils.getObsblockName(), 
                          db, tau=weather[1], phase=weather[3], isuseable=isUseable)
                else:
                   sn.addTimeDatabase(sid, tint_actual_target/60.0, utstart,
                          len(commands.currentAntennaNumbers()), 
                          utils.getObsblockName(), 
                          db, tau=weather[1], phase=weather[3], isuseable=isUseable)

        # Stop if nothing was observed or we have reached the time limit.
        if not utils.continueObservations(isScience=True) or tint_actual_cycle == 0 and \
