...
QuadraticInterpolator::QuadraticInterpolator()
{
  ...
  if(pthread_mutex_init(&mutex_, NULL)) {
    LogStream errStr;
    errStr.appendSysError(true, "pthread_mutex_init()");
    throw Error(errStr);
  }
...
void QuadraticInterpolator::lock()
{
  if(mutexIsReady_) {
    ...
    LogStream errStr;
    errStr.appendMessage(true, "Mutex is not intialized");
    throw Error(errStr);
  }
}
...
void QuadraticInterpolator::unlock()
{
  if(mutexIsReady_) 
    if(pthread_mutex_unlock(&mutex_)) {
      LogStream errStr;
      errStr.appendSysError(true, "Error in pthread_mutex_unlock\n");
      throw Error(errStr);
    }
}
...
bool QuadraticInterpolator::tryLock()
{
  if(mutexIsReady_) {
	...
	LogStream errStr;
	errStr.appendSysError(true, "Error in pthread_mutex_trylock\n");
	throw Error(errStr);
      }
    }
