/** RSTATUSCM	- display all aspects of CARLSTROM receiver			*/
/*: basic								*/
/*+
	RSTATUSCM ANT=xxx

	RSTATUSCM displays all quantities that pertain to an individual
...
main(int argc, char *argv[])
{
  ...
  float mbanderr, mbandif,mloopg,mloopmon ;
  int d1069 =  1069, d1068 =  1068, x66m = 0x66-256, d1070 =  1070 ;
  int x10 = 0x10-256;
  int cmout;
  ...
  int d1080 =  1080, d1021 =  1021, d1009 =  1009, d1016 =  1016, x9c = 0x9c ;
  ...
  int mharm,nharm;
	...
	printf("RSTATUS (version 1.0 12-oct-92)\n") ;
	if (ant ==3) { 
...
/*  --	COMMON BLOCK	*/
	  ...
	  sprintf(mess,"FREQS: LO1=%7.3f  Mharm= %d  Xfreq=%6.3f  Synth=%.6lf   ", \
		lo1,mharm,xfreq,refmhz) ;
...
/*  --	POWER SUPPLIES	*/
	  ...
	  sprintf(mess,"PWR:   +15= %4.1f  -15= %4.1f  +5= %3.1f    Tuneenable " \
...
/*  --	CM BAND		*/
	  ...
	  cmout = tpeek_(&ant,&x10,&two,error);
	  if(mmlockstat_(&ant)) strcpy(lock,"Lock") ;  else strcpy(lock,"BAD ") ;
	  sprintf(mess,"CMLCK: Status= %s  Err volts= % .2f  IF Level= %.2f "\
		    " CMYIG tune = %4d       ",lock,mmerrv_(&ant),mmlev_(&ant),cmout) ;
...
/*
	  ...
	  mvopb = atod_(&ant,&d1051,&raw,units,name,error) ;
*/
	  sprintf(mess,"CMLCK: 70_MHz_Ref= %s  Sweep= %s                         ",\
	      refok,sweepon) ;
	  move(9,0) ; addstr(mess) ;

/*  --	IF; note that for cm rcvrs amb load is mounted on pol frame	*/
	  band = bands[(int)(bitsin&0x3) ] ;
	  if ((bitsin&0x20) !=0) strcpy(ifon,"ON") ; else strcpy(ifon,"OFF") ;
	  if ((bitsin&0x10) !=0) strcpy(atten,"0DB") ; else strcpy(atten,"10DB") ;
	  totpow = atod_(&ant,&d1094,&raw,units,name,error) ;
	  pol = atod_(&ant,&d1081,&raw,units,name,error) ;
	  polpos_(&ant,&polcode);
	  sprintf(mess,"2ndIF: Amp= %s  Atten= %s  Power=%7.4f" \
		"  Calib= %5.0f (%c)",
		ifon,atten,totpow,pol,polcode) ;
...
/*  --	TEMPS	*/
	  ...
	  strcpy(t3,therm(&ant,&d1016)) ;
/*	  strcpy(t4,therm(&ant,&d1013)) ;  not needed since Tbc doesn't exist */
	  strcpy(t5,therm(&ant,&d1012)) ;
	  sprintf(mess,"TEMPS: Htr=%4.2f  Tplate=%s  T2ndIF=%s "\
		" Tharm=%s  Tamb=%s",htrmon,t1,t2,t3,t5) ;
...
/*
	  ...
	  strcpy(t6,therm(&ant,&d1087)) ;
	  sprintf(mess,"DEWAR: sensor 1= %s  2= %s  3= %s  4= %s  5= %s  6= %s   ",
		  t1,t2,t3,t4,t5,t6) ;
*/
	  sprintf(mess,"DEWAR: sensor 1= %s  2= %s                           ", t1,t2) ;
	  move(17,0) ; addstr(mess) ;

/*
	  htrout_stg2 = (float)(tpeek_(&ant,&x62m,&two,error)) * 12.1/2048. ;
	  htrmon_stg2 =  atod_(&ant,&d1091,&raw,units,name,error)  ;
	  htrout_stg3 = (float)(tpeek_(&ant,&x54m,&two,error)) * 12.1/2048. ;
	  htrmon_stg3 =  atod_(&ant,&d1005,&raw,units,name,error)  ;
	  ...
	  if ((ant==6) || (ant==8)) {
	  	...
	  	sismxr = (float)(tpeek_(&ant,&x56m,&two,error)) * 20./2048. ;
	  	...
	  	sismxv =  atod_(&ant,&d1049,&raw,units,name,error) ;
	  	sismxc =  atod_(&ant,&d1006,&raw,units,name,error) ;
