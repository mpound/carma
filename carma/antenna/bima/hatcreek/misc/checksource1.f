...
C----------------------------------------------------------------------------
      ...
      real*8 lo,lst,u_ns(nants),v_ns(nants),frequp,freqlow,ut,freq
      integer ticks
      real*4 j1xbyx,az
      ...
      CALL GET_SOURCE_NOLOCK(SRC,CAT,EPOCH,S,TJD,ERROR)
      IF(ERROR(1:2).NE.'OK') GOTO 99
...
994   FORMAT('              temperature: ',F6.1,
     ...
     *   		(EXP(.04801*(FREQUP)/S.PLTB)-1.)
	    FLXLWR=S.PLMAJ*S.PLMIN*1158.1*(FREQLOW)**3/
     *          	(EXP(.04801*(FREQLOW)/S.PLTB)-1.)
	  ...
	  type *
	  type *,'ANT      FLUX (Up+Low)/2 \\ -SQRT(U*U+V*V) '
	  TYPE *,'       1        2        3        4        5'//
     *     	'        6        7        8        9'
     ...
     *	   		+(S.PLMIN*(UB*SINI+VB*COSI))**2)
		PLFLX=ABS(J1XBYX(SNGL(BETA_NS*FREQUP)))*FLXUPR
     *			   +ABS(J1XBYX(SNGL(BETA_NS*FREQLOW)))*FLXLWR
