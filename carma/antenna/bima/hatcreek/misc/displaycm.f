...
c   sep 96 rp - this version (displaycm) only shows ants 2,4,6,7,8,9
c	jun97 wh - new ant 1
c----------------------------------------------------------c
...
c  --  the 7 windows are: time,weather,point,freq,corr,ants,program
	...
	integer irow(8)    /  0,  0,    4,    1,   2,   4,   12, 12/
...
c----------------------------------------------------------------c
	...
	WRITE(BUFFER,'(A,I9)',ERR=99) 'ANT: ',ANTS
99	CALL MOVE(%val(IROW),%val(ICOL))
...
c----------------------------------------------------------------c
	...
	WRITE(BUFFER,'(A,F13.8,A,F10.4,A,F7.3,A,F13.6)',ERR=99)
...
c----------------------------------------------------------------c
	...
	2	'4000'X,'8000'X,'10000'X,'20000'X/
	integer*4 cmask(2) /8,16/
	integer lxladd(9) /'5f'x,'5f'x,'1f'x,'5f'x,'5f'x,'5f'x,'5f'x,'5f'x,'5f'x/
	integer lmadd(9) /'6e'x,'6e'x,'5f'x,'6e'x,'6e'x,'6e'x,'6e'x,'6e'x,'6e'x/
	integer lm(9) /1,1,2,1,1,1,1,1,1/
	...
	antx = 0
	DO ANT=1,10
	 if ((ant.eq.1).or.(ant.eq.3).or.(ant.eq.5).or.(ant.eq.10)) goto 57
	 antx = antx + 1
	 DO JAX=1,2 
...
99	CALL MOVE(%val(IROW+antx),%val(ICOL))
c	if (ant.eq.2) buffer = ' '
	CALL ADDSTR(BUFFER(:IWIDTH))
57	continue
	END DO
	IF (DEAD.GT.10) THEN
