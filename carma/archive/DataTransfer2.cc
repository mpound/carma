...
namespace { //anonymous
    const size_t FRAMES_PER_MINUTE = 120;
}
...
using namespace carma::monitor;
    ...
    DataTransfer::DataTransfer(map<string, string> arv, 
            const util::frameType startFrame,
            const util::frameType pollInterval,
            const bool emulate ) :
        ...
        numAHXferred_(0),
        numMPXferred_(0),
        numVisbrickXferred_(0),
        numQualityXferred_(0)
    {
        ...
        */
        ...
        // Start the autowriter and set initial values for the
        // monitor subsystem points.  Don't set 
        // dataTransfer().transferTime(lastXferTime_) 
        // because it is a persistent monitor point and it is 
        // useful to know the value from the last time the program 
        // was run. The rest of these we want to zero out
        // when the program is started.
        dataflow_.startAutoWriter(0.1);
        dataflow_.dataTransfer().uptime().setValue(0);
        dataflow_.dataTransfer().numTransfer().setValue( 0 );
        dataflow_.dataTransfer().numVisbrickTransfer().setValue( 0 );
        dataflow_.dataTransfer().numHeaderTransfer().setValue( 0 );
        dataflow_.dataTransfer().numMonitorTransfer().setValue( 0 );
        dataflow_.dataTransfer().numQualityTransfer().setValue( 0 );
        dataflow_.dataTransfer().filesPerHour().setValue( 0 );
        dataflow_.dataTransfer().filesPerDay().setValue( 0 );

        // Insert the starting value for the transfer rate data
        fileXferRateMap_.insert( make_pair( startFrame_, 0 ) );
                             ...
                             const string & filter)
    {
      ...
      setParams(p, filter);
      xferFileType xft = FILE_NONE; 
      if(filter.compare("astrohdr_")==0) {
          xft = FILE_ASTROHEADER;
      } else if(filter.compare("VisBrickData_")==0) {
          xft = FILE_VISBRICK;
      }

      const size_t num = 3;  //the number of files in each batch
      size_t begin = 0;
      size_t end = num;
    ...
    int DataTransfer::move_qr(const vector<string> & p, const string & qrDir , const string & filter)
    {
      ...
      while(begin < listSize)
      {
        ...
        if(n > 0) {
          vector<string> v = jniObj->call_update(aBatch);
          size_t length = v.size();
          setLastTransferred(length, FILE_QUALITY );
          if(length > 0)
          {
            //write to transfer db and update done-list
    ...
    int DataTransfer::move_sza(const vector<string> & p, const vector<string> & dirs, const string & filter)
    {
      ...
      while(begin < listSize)
      {
        ...
        if(n > 0) {
          vector<string> v = jniObj->call_update(aBatch);
          size_t length = v.size();
          setLastTransferred( length, FILE_SZA );
          if(length > 0)
          {
            //write to transfer db and update done-list
            writeTransferDB(v, toAppend);
          ...
          while(begin < listSize)
          {
        	  ...
        	  if(n > 0) {
				  ...
				  if(FileUtils::exists(tar)) {
					  ...
					  size_t length = v.size();
                      setLastTransferred( length, FILE_MONITOR );
...
void 
DataTransfer::setLastTransferred( const size_t length, const xferFileType type )
{
    if ( type == FILE_NONE ) return;
    lastXferTime_ = Time::computeClosestFrame();
    fileXferRateMap_.insert( make_pair( lastXferTime_, length ) );
    numFilesXferred_ += length;
    switch ( type ) {
        case FILE_ASTROHEADER :
            numAHXferred_ += length;
            break;
        case FILE_MONITOR:
            numMPXferred_ += length;
            break;
        case FILE_QUALITY:
            numQualityXferred_ += length;
            break;
        case FILE_VISBRICK:
            numVisbrickXferred_ += length;
            break;
        default:
            return;
    }
...
size_t
DataTransfer::fileTransferRate( const size_t intervalInMinutes )
{
    // Find first time index in the map that is equal to or
    // greater than the time of the last tranfer minus the requested interval.
    // Subtract off one poll interval (equals one index) to ensure we capture the 
    // full interval.  If the process has not been running as long as 
    // the interval, this will return the total number of files transferred.

    const size_t intervalInFrames = (intervalInMinutes * FRAMES_PER_MINUTE) ;
    frameType startOfInterval = lastXferTime_ - intervalInFrames - pollInterval_ ;
    map<frameType,size_t>::iterator iter 
        = fileXferRateMap_.lower_bound( startOfInterval );
    map<frameType,size_t>::const_iterator iEnd = fileXferRateMap_.end();

    frameType actualStart = iter->first;
    frameType interval = lastXferTime_ - actualStart;
    if ( interval <= intervalInFrames ) return numFilesXferred_;
    ...
    // Returning the sum is close enough, we may be off by +/- one 
    // poll interval which is typically 2 minutes.
    return numFiles;
}

void 
DataTransfer::updateMonitorPoints()
{
    frameType curFrame = Time::computeClosestFrame();
    int uptime = ( curFrame - startFrame_ ) / FRAMES_PER_MINUTE ; // convert to minutes
    dataflow_.dataTransfer().uptime().setValue( uptime );
    dataflow_.dataTransfer().transferTime().setValue( lastXferTime_ );
    dataflow_.dataTransfer().numTransfer().setValue( numFilesXferred_ );
    dataflow_.dataTransfer().numVisbrickTransfer().setValue( numVisbrickXferred_ );
    dataflow_.dataTransfer().numHeaderTransfer().setValue( numAHXferred_ );
    dataflow_.dataTransfer().numMonitorTransfer().setValue( numMPXferred_ );
    dataflow_.dataTransfer().numQualityTransfer().setValue( numQualityXferred_ );
    dataflow_.dataTransfer().filesPerHour().setValue( fileTransferRate( 60 ));
    dataflow_.dataTransfer().filesPerDay().setValue( fileTransferRate( 1440 ));
