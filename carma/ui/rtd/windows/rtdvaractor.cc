/** @file
 * Gets data from the carma varactor subsystem and displays.
 *
 * <dl><dt><b>Author </b></dt><dd>Andy Beard </dl>
 * $Revision: 1.5 $
 * $Date: 2013/11/19 03:41:15 $
 * $Id: rtdvaractor.cc,v 1.5 2013/11/19 03:41:15 iws Exp $
 *
 * $CarmaCopyright$
 */
...
makeHelp( const string & antString ) {
    ostringstream ost;
    ost
    << "       " << antString << " ANTENNA VARACTOR HELP\n\n" 
    << "Status of the " << antString << " antenna varactor CAN module. "
...
template < typename S >
void 
populateMonitorContainerVecs( vector< MonitorContainer * > & varactors,
                              vector< MonitorContainer * > & xacs,
                              S & monitorSubsystem )
{
    // Deliberately leaks
    MonitorContainer * varactor = new MonitorContainer("Varactor");
    varactor->add( monitorSubsystem.gunn1cm().state() );
    varactor->add( monitorSubsystem.gunn1cm().varactor() );
    varactors.push_back( varactor );

    // Deliberately leaks
    MonitorContainer * xac = new MonitorContainer("Xac"); 
    xac->add( monitorSubsystem.gunn1cm().state() );
    xac->add( monitorSubsystem.gunn1cm().xac() );
    xacs.push_back(xac);
}
...
Program::main() {
    ...
    // Create a dislay
    MonitorDisplay display( antString + " Antenna Varactor Status" );
    display.setSpecificHelp( antString + " Varactor Help", 
                             makeHelp( antString ) );

    int width = AUTO_SIZE;  
    vector<string> columnLabel = getAntNames( antType );
    vector<MonitorContainer*> varactors;  
    vector<MonitorContainer*> xacs;  
        ...
        case BIMA:
            {
                for ( int i = 0; i < BimaSubsystem::COUNT; ++i) {
                    ...
                    populateMonitorContainerVecs( varactors, xacs, bima );
                }
            }
            break;
        case OVRO:
            {
                for ( int i = 0; i < OvroSubsystem::COUNT; ++i) {
                    ...
                    populateMonitorContainerVecs( varactors, xacs, ovro );
                }
            }
            break;
        case SZA:
            {
                for ( int i = 0; i < SzaSubsystem::COUNT; ++i) {
                    ...
                    populateMonitorContainerVecs( varactors, xacs, sza );
                }
            }
            break;
        default:
    ...
    MonitorSingleTableFolderPtr folder1(new MonitorSingleTableFolder(
                "varactor", columnLabel, varactors, width, 2));
    MonitorSingleTableFolderPtr folder2(new MonitorSingleTableFolder(
                "varactor (xac)", columnLabel, xacs, width, 2));
